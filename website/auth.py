...
def forgot():
    ...
    if request.method == 'POST':
        ...
        if data:
            ...
            con.commit()
            email_alert("Reset Password", "Hi there. To reset your password, click the link: http://127.0.0.1:5000/reset/"+str(token), email)
...
def comment(id):
    ...
    if "email" in session:
        ...
        if request.method == "POST":
            ...
            email_alert("Notification", "Hi "+author+". "+email+" has been comment to your post. Click the link to redirect. http://127.0.0.1:5000/blog/"+str(id), eaddress)
